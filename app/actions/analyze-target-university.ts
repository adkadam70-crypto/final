 ...
 * Catalog-only — see the comment on the `!matched` branch below for why the
 * old "search the web for any school you type" path was removed.
 *
 * Repeat requests for the same school against the same saved profile are
 * served from the last stored analysis (see the cache check below) instead
 * of asking the AI again — a generative call has no reason to reproduce the
 * exact same probability twice, so without this a student re-opening the
...
export async function analyzeTargetUniversity(
  ...
  try {
    ...
    // The old fallback here (web-search + AI-estimate any school typed in,
    // catalog or not) got abused — people typing arbitrary/fake names just
...
${
  dreamPriorityMode
    ? restrictToCountry === 'US'
      ...
      : `\nIMPORTANT — actionSteps ordering for this request: this is feeding a per-college task checklist, not general advice. This school is NOT in the US — do not mention the Common App, FERPA, or any other US-application-specific process; those are meaningless here. actionSteps[0] MUST specifically address a real requirement unique to THIS school beyond the generic country-wide baseline already covered elsewhere (e.g. a required supplemental essay, portfolio, interview, entrance exam, or specific document this particular school asks for) — grounded only in what you're actually confident is real; if genuinely uncertain of the specifics, say a school-specific requirement likely exists and to check the school's own admissions page rather than inventing one. The remaining actionSteps (up to 3 more) must be concrete, specific tasks tied directly to an actual gap in THIS student's profile relative to THIS school (a specific missing score, requirement, or activity type) — never generic advice like "improve your essays" or "get better grades."\n`
    : ''
}
