...
export interface ProgressiveFluxLoaderProps {
  ...
  showLabel?: boolean;
  /**
   * CSS background for the bar fill. Defaults to the site's turquoise/teal
   * flux gradient (driven by the `--flux-from` / `--flux-to` CSS variables,
   * which default to the theme's `--primary` teal). Pass any CSS background
...
// Site "flux" palette — a turquoise/teal fill matching the app's --primary
// accent (oklch(0.74 0.17 178), a vivid teal/cyan). Read from CSS variables
// with theme-matched defaults, so the bar can be recolored per instance
// without touching the component: set `--flux-from` / `--flux-to`, or
// override the whole fill with the `gradient` prop. The surrounding track and
// label stay on shadcn theme tokens, so the loader still adapts to light and
// dark. These are component-level custom properties, so the v3 build leaves
// them untouched and the fill renders identically on Tailwind v3 and v4.
const FLUX_FROM = "var(--flux-from, oklch(0.74 0.17 178))";
const FLUX_TO = "var(--flux-to, oklch(0.85 0.14 178))";
...
// and a deep-teal inset for depth.
const BAR_SHADOW = `0 0 18px color-mix(in oklab, ${FLUX_FROM} 55%, transparent), 0 0 32px color-mix(in oklab, ${FLUX_TO} 40%, transparent), inset 0 1.5px 0 rgba(255, 255, 255, 0.5), inset 0 -2px 3px rgba(0, 60, 60, 0.35)`;
...
function FluxLabel({ label, reduced, className }: FluxLabelProps) {
  const base = cn(
    "absolute inset-0 flex items-center justify-center text-center text-xs font-medium tracking-tight text-muted-foreground",
    className,
  );
        ...
        <span className="inline-flex">
          {label.split("").map((char, index) => (
            <motion.span
              ...
              // A lone space character inside an inline-block collapses to
              // zero width under normal CSS whitespace rules — `pre`
              // preserves it, otherwise every word in the label runs
              // together (e.g. "reading your profile" rendered as
              // "readingyourprofile").
...
export function ProgressiveFluxLoader({
  ...
  return (
    <div
      className={cn(
        "mx-auto flex w-full max-w-xs flex-col items-center gap-2",
        className,
      )}
    >
      {showLabel && (
      ...
      <div
        className={cn(
          "relative h-1.5 w-full overflow-hidden rounded-full bg-muted shadow-[inset_0_2px_3px_rgba(0,0,0,0.09),inset_0_-1px_2px_rgba(255,255,255,0.7)] dark:shadow-[inset_0_2px_3px_rgba(0,0,0,0.45),inset_0_-1px_2px_rgba(255,255,255,0.05)]",
          barClassName,
        )}
        role="progressbar"
        aria-valuemin={0}
