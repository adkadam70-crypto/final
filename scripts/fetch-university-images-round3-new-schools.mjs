// Same image-fetch pipeline as fetch-university-images-round2.mjs, scoped
// to just the 58 universities added in tonight's US program-ranking
// gap-fill batches (this session), rather than the full backlog of
// pre-existing rows still missing an image.
//
// Usage: node --env-file=.env.local scripts/fetch-university-images-round3-new-schools.mjs
...
async function main() {
  const rows = await sql`
    ...
    WHERE "imageUrl" IS NULL AND "createdAt" > now() - interval '6 hours'
    ORDER BY id
  `
  console.log(`${rows.length} newly-added universities need an image`)
